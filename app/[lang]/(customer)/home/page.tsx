...
function Page() {
  ...
  const [specialOfferData, setSpecialOfferData] = useState<Property[] | null>(
    null
  );
  const [allHouseData, setAllHouseData] = useState<Property[] | null>(null);

  const [isLoadingPromotion, setIsLoadingPromotion] = useState(false);
  const [isLoadingCategory, setIsLoadingCategory] = useState(false);
  const [isLoadingSpecialOffers, setIsLoadingSpecialOffers] = useState(false);
  const [isLoadingAllHouse, setIsLoadingAllHouse] = useState(false);
  ...
  const fetchCategoryData = useCallback(async () => {
    try {
      ...
      const data = await categoryListHouse();
  ...
  const fetchSpecialOfferData = useCallback(async () => {
    try {
      ...
      const data = await specialOffers();
  ...
  const fetchAllHouseData = useCallback(async (searchTerm: string) => {
    try {
      setIsLoadingAllHouse(true);
      const data = await allHouse(searchTerm);
      setAllHouseData(data);
    } catch (error) {
      console.error("Error fetching house data:", error);
      setAllHouseData(null);
    } finally {
      setIsLoadingAllHouse(false);
    }
  }, []);
  ...
  }, [fetchPromotionData, fetchCategoryData, fetchSpecialOfferData]);

  // Load house data when search changes
  useEffect(() => {
    fetchAllHouseData(debouncedSearch);
  }, [fetchAllHouseData, debouncedSearch]);
